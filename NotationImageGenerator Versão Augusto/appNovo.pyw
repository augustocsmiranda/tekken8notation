...
class VirtualKeyboardApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Tekken - Notation Combo Image Generator")


        # Add Entry widget for string input
        self.string_input = tk.Entry(root)
        self.string_input.grid(row=11, column=0, columnspan=2)
        # Bind the KeyRelease event to the process_string_input method
    ...
    def process_string_input(self, event):
            ...
            sequences = [i.upper() for i in input_string.split()]
            ...
            for sequence in sequences:
                for move_dict in self.MoveDict:
                    if sequence == move_dict["Move"].upper():
                        self.selected_images_names.append(move_dict["Image"])
                        break
            # for image in self.selected_images:
            #     self.selected_images_names.append(self.get_file_name(image))
            #     # print(self.selected_images_names[0][3:][:-4])
            old_selected_images = self.selected_images.copy()
            self.selected_images = [os.path.join(self.selected_assets, image_name) for image_name in self.selected_images_names]
    ...
    def create_widgets(self):
        ...
        text_entry.grid(row=i + 2, column=3, columnspan=2)
        ##############################
